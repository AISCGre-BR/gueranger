...
import { columns, SortIndicator, type ManuscriptRow } from "../lib/columns";
        ...
        <div className="flex items-center gap-4">
          ...
          {sourcesSucceeded.length > 0 && (
            <span className="text-xs text-slate-400">
              Results from: {sourcesSucceeded.join(", ")}
            </span>
          )}
        </div>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className="bg-slate-50">
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="select-none px-4 py-3 text-left text-xs font-normal uppercase tracking-wide text-slate-500"
            ...
            {table.getRowModel().rows.map((row, i) => (
              <tr
                ...
                className={`
                  ...
                  ${row.getIsSelected() ? "border-l-3 border-l-blue-600 bg-blue-50" : ""}
